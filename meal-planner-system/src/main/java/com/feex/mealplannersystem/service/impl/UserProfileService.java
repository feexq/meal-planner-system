...
import com.feex.mealplannersystem.dto.profile.ProfileSummaryDto;
...
public class UserProfileService {
    ...
    public ProfileSummaryDto getProfileSummary(UserEntity user) {
        ...
        return ProfileSummaryDto.builder()
                ...
                .targetWeightKg(targetWeightKg) // ОНОВЛЕНО
    ...
    private void deleteOldAvatar(String oldUrl, Long userId) {
        ...
        try {
            ...
            if (prefixIndex == -1) {
                log.warn("Невірний формат URL аватара для user={}: {}", userId, oldUrl);
                return;
            }
            ...
            if (isDeleted) {
                log.info("Старий аватар успішно видалено з Azure: {}", blobName);
            } else {
                log.warn("Аватар не знайдено в Azure для видалення (можливо, вже видалений): {}", blobName);
            }

        } catch (Exception e) {
            log.error("Помилка під час видалення старого аватара для user={}. Причина: {}", userId, e.getMessage(), e);
        }
    }
